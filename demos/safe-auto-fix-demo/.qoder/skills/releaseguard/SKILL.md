...
Use the local evidence-backed review flow for every non-safe finding. `SAFE`
debug configuration remains eligible only after the user explicitly authorizes
the bounded edit. High/Critical findings remain `NEEDS_REVIEW`; Qoder must not
approve, reject, defer, or mark a false positive on its own. Use:

```powershell
releaseguard review --project .
releaseguard approve <FINDING_ID> --project . --reason "Human authorization"
releaseguard reject <FINDING_ID> --project . --reason "Reason"
releaseguard defer <FINDING_ID> --project . --reason "Reason"
releaseguard false-positive <FINDING_ID> --project . --reason "Required evidence"
releaseguard remediate <FINDING_ID> --project .
releaseguard dashboard --project .
```

Approval is not resolution. The approval must match the finding fingerprint,
audit run, project snapshot, and explicit allowed scope. A stale snapshot or
out-of-scope diff fails closed. Only a real deterministic re-audit may produce
`RESOLVED`; AI/Qoder cannot set the gate or hide a remaining finding. Keep raw
credentials out of prompts, transcripts, reports, logs, dashboard, and GUID
